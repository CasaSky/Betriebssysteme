...
public class OperatingSystem {
	 ...
	 * 
	 * @param processSize
	 * 		die Gr��e des Prozess-Hauptspeicherbedarfs in Byte
	 *
	...
	public synchronized int read(int pid, int virtAdr) {
		int VSN = getVirtualPageNum(virtAdr);
		PageTableEntry pte = getProcess(pid).pageTable.getPte(VSN);

		if (pte == null) {
			testOut("OS: read " + pid + " +++ Seitennr.: " + VSN
					+ " in Seitentabelle nicht vorhanden");
			return -1;
		} else if (!pte.valid) {
			pte = handlePageFault(pte, pid);
		...
		return readFromRAM(pte.realPageFrameAdr);
	}

...
public class PageTable {
	...
	private PageTableEntry randomAlgorithm(PageTableEntry newPte) {
		...
		int randomInt = ThreadLocalRandom.current().nextInt(0, pteRAMlist.size());

		PageTableEntry foundPe = pteRAMlist.remove(randomInt);
		foundPe.valid = false;
		newPte.valid = true;
		pteRAMlist.add(newPte);

		//PageTableEntry foundPe = pageTable.stream().filter(e -> e.valid).findAny().get();
		return foundPe;
	}
